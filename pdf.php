...
function CrearCertificacionPDF($id_cliente, $fechaInicial, $fechaFinal, $db){
    if($id_cliente!=0){
        $sql = "SELECT id_factura, nit, nombre, direccion,factura_uuid, total_factura FROM factura WHERE `factura`.`cliente_id` = ? and fecha BETWEEN  ? and ? ORDER BY NIT";  
        // Listar Empresas
        $stmt = $db->prepare($sql);
        ...
        function BasicTable($data)
        {
            ...
            $pdf->Cell(2,0.6,"NIT");
            $pdf->Cell(6,0.6,"UUID");
            $pdf->Cell(3,0.6,"NOMBRE",0,0,'R');
            $pdf->Cell(2,0.6,"DIRECCION",0,0,'R');
            $pdf->Cell(2,0.6,"FECHA",0,0,'R');
            $pdf->Ln();
            // detalle factura
            foreach($data as $row){
                $pdf->Cell(1,0.6,"-");
                $pdf->Cell(2,0.6,$row['nit']);
                $pdf->Cell(6,0.6,$row['factura_uuid']);
                $pdf->Cell(3,0.6,$row['nombre'],0,0,'R');
                $pdf->Cell(2,0.6,$row['direccion'],0,0,'R');
                $pdf->Cell(2,0.6,$row['total_factura'],0,0,'R');
                $pdf->Ln();
            }
            $pdf->Ln();
